...
var webpackConfig = {
  ...
  resolve: {
    modules: [
      ...
      .concat(getFullLowerCaseDirectories(path.resolve(__dirname, "app", "components")))

      .concat(path.resolve(__dirname, "app", "enhancers"))
      .concat(getFullLowerCaseDirectories(path.resolve(__dirname, "app", "enhancers")))

      .concat(path.resolve(__dirname, "app", "models"))
      .concat(getFullLowerCaseDirectories(path.resolve(__dirname, "app", "models")))
  ...
  node: { // required by lib/timecamp-v3-javascript-client
    fs: 'empty',
    net: 'empty',
    tls: 'empty',
    __dirname: true
...
function getFullLowerCaseDirectories(forPath) {
  return getLowerCaseDirectories(forPath)
    .map(relativeDir => path.resolve(forPath, relativeDir))
}

function getLowerCaseDirectories(forPath) {
  return getDirectories(forPath)
    .filter(wordIsLowercase)
}

function getDirectories(forPath) {
  console.log(__dirname)
  return fs.readdirSync(forPath)
